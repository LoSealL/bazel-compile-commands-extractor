...
def _ensure_external_workspaces_link_exists(symlink_prefix='bazel-'):
    ...
    if not os.path.lexists(f'{symlink_prefix}out'):
        log_error(f">>> //{symlink_prefix}out is missing. Please remove --symlink_prefix and --experimental_convenience_symlinks, so the workspace mirrors the compilation environment.")
